...
| **项目名称** | 多签跨链桥系统 |
| **开始日期** | 2025-11-12 |
| **当前状态** | M2/M3阶段 - SVM合约已完成 ✅，EVM合约核心功能已完成 🟡 |
| **最后更新** | 2025-11-15 |
...
| M4 | Relayer服务开发 | ⚪ 未开始 | 2025-12-31 | 中继服务开发（事件监听、双算法签名、交易提交） |
...
- [ ] 实现事件监听模块
  - [ ] EVM 链事件监听（使用 Ethers.js）
  - [ ] SVM 链事件监听（使用 Solana Web3.js）
- [ ] 实现双算法签名模块
  - [ ] 私钥管理（Ed25519 + ECDSA 双密钥对）
  - [ ] Ed25519 签名（用于提交到 SVM）
  - [ ] ECDSA 签名（用于提交到 EVM）
  - [ ] 数据格式转换（Borsh ↔ JSON）
- [ ] 实现交易提交模块
  - [ ] EVM 交易提交（含 ECDSA 签名）
  - [ ] SVM 交易提交（含 Ed25519Program 指令）
- [ ] 配置管理
- [ ] 日志和监控
- [ ] 错误处理和重试机制
- [ ] 单元测试和集成测试
...
- [ ] **M2阶段收尾工作**
  - [x] 设置 Foundry 环境 ✅ 已完成
  - [x] 实现 EVM 合约（与 SVM 对等） ✅ 已完成
  - [x] 编写 EVM 合约测试 ✅ 已完成（31/41通过）
  - [ ] 修复剩余 10 个测试用例
  - [ ] 达到 90% 测试通过率
  - [ ] EVM 合约代码审查和优化
- [ ] **M3阶段收尾工作**
  - [x] SVM合约核心功能实现 ✅ 已完成
  - [x] SVM合约测试（45/48通过，93.75%） ✅ 已完成
  - [x] Ed25519 签名验证 ✅ 已完成
  - [ ] 本地测试网部署验证
