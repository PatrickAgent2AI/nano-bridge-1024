...
- [x] 修复 TC-107 测试（单个签名不应解锁）
- [ ] 接收端合约签名验证功能测试（TC-107 ~ TC-113，进行中，需要完整 ECDSA 验证）
- [ ] 本地测试网部署验证
- [ ] 代码审计（内部）
...
✅ **完成 SVM 接收端合约基础功能实现**
  ...
  - 📝 **设计调整：支持无限请求和最多21个relayer，使用PDA账户存储签名记录**

### 下周计划（2025-11-14 ~ 2025-11-20）

- [ ] 完成 SVM 接收端合约功能
  - [x] 初始化功能（initialize_receiver）
  - [x] 配置功能（configure_source）
  - [x] Relayer 白名单管理（add_relayer, remove_relayer, is_relayer）
  - [x] ECDSA 公钥存储机制
  - [x] 签名提交和阈值检查（submit_signature）
  - [x] 实现多签阈值检查（> 2/3 relayer）
  - [x] 实现 nonce 防重放机制
  - [ ] 实现完整的 ECDSA 签名验证逻辑（解析 DER 格式，使用 secp256k1_program）
  - [ ] 让接收端合约测试通过（TC-107 ~ TC-113）
- [ ] 实现集成测试功能（IT-001 ~ IT-004）
- [ ] 实现安全测试功能（ST-001 ~ ST-005）
- [ ] 实现性能测试功能（PT-001 ~ PT-004）
- [ ] 本地测试网部署验证
- [ ] 开始 EVM 合约开发环境搭建
