...
- [ ] 接收端合约签名提交功能测试（TC-107 ~ TC-113，进行中）
- [ ] 本地测试网部署验证
- [ ] 代码审计（内部）
...
- [ ] 实现 SVM 接收端合约功能
  ...
  - [ ] Relayer 白名单管理（add_relayer, remove_relayer, is_relayer）
  - [ ] 签名验证和解锁功能（submit_signature）
  - [ ] 实现 ECDSA 签名验证逻辑
  - [ ] 实现多签阈值检查（> 2/3 relayer）
  - [ ] 实现 nonce 防重放机制
  - [ ] 让接收端合约测试通过（TC-101 ~ TC-113）
...
2. **Solana 账户大小限制问题**（2025-11-14）
   ...
   - 解决：优化账户结构，减少到 3252 字节；设计 PDA 方案支持无限请求
   - 状态：✅ 已解决
