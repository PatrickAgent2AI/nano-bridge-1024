...
fn verify_ecdsa_signature(
    ...
    // First SHA256 hash using Solana's hash function
    let message_hash1 = solana_program::hash::hash(&json_bytes);
    
    // Second SHA256 hash (as test does crypto.createSign("SHA256").update(hash).sign())
    let message_hash2 = solana_program::hash::hash(message_hash1.as_ref());
    ...
    // DER signatures don't include recovery_id, so we try all 4 possible values (0-3)
    let mut verified = false;
    for recovery_id in 0..4u8 {
        if let Ok(recovered_pubkey) = solana_program::secp256k1_recover::secp256k1_recover(
            &message_hash,
            recovery_id,
            &sig_bytes
        ) {
            ...
            if &recovered_pubkey_full == ecdsa_pubkey {
                verified = true;
                break;
            }
        }
