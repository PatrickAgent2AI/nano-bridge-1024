...
Relayer 是跨链桥系统的核心中继组件，负责监听质押事件、验证签名并将签名提交到接收端合约。

### 实现状态

**✅ M4 阶段完成（2025-11-16）**
- ✅ **S2E Relayer** (SVM→EVM)：完整实现并端到端验证成功
- ✅ **E2S Relayer** (EVM→SVM)：完整实现并运行中
...
│  ├───────────────────────┤         ├──────────────────────────────┤ │
│  │ 监听: 1024chain       │         │ ┌─────────────────────────┐ │ │
│  │ 方式: HTTP RPC 轮询   │         │ │ e2s-listener            │ │ │
│  │ 签名: ECDSA           │         │ │ 监听: Arbitrum (EVM)    │ │ │
│  │ 格式: JSON+SHA256     │         │ │ 签名: -                 │ │ │
│  │ 提交: Arbitrum        │         │ │ 输出: 文件队列          │ │ │
│  │ 端口: 8081            │         │ └─────────────────────────┘ │ │
│  │ 状态: ✅ 运行中       │         │           ↓                  │ │
...
│  │  - GET /health         - 健康检查                              ││
│  │  - GET /status         - 运行状态                              ││
│  │  - GET /metrics        - 性能指标（Prometheus 格式）           ││
│  └─────────────────────────────────────────────────────────────────┘│
└──────────────────────────────────────────────────────────────────────┘
```
...
### Phase 1: 核心功能（M4）✅ **已完成（2025-11-16）**
...
- [x] SVM 事件监听器 ✅
  ...
  - [x] Anchor 事件日志解析（base64 + Borsh）
- [x] ECDSA 签名器 ✅
  - [x] JSON 序列化（bytes32 小写 hex）
  - [x] SHA-256 + EIP-191
  - [x] secp256k1 签名（65字节）
- [x] EVM 交易提交器 ✅
  - [x] ethers-rs 集成
  - [x] submitSignature 调用
  - [x] 交易确认等待
- [x] HTTP API ✅（端口 8081）
- [x] **端到端测试** ✅
  - [x] USDC 余额变化验证：+0.001001
  - [x] 交易哈希：`0xcdad...46ec`
...
- [x] e2s-listener ✅
  ...
  - [x] 保存到文件队列
- [x] e2s-submitter ✅
  - [x] Ed25519 签名器（Borsh + Ed25519）
  - [x] Ed25519Program 预验证指令
  - [x] Anchor submitSignature 指令
  - [x] PDA 账户推导
  - [x] 交易模拟和发送
- [x] HTTP API ✅（端口 8082）
- [x] **服务运行中** ✅
  - [x] listener 监听 EVM 事件
  - [x] submitter 处理队列
